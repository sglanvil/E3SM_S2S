...
% /glade/campaign/cesm/development/cross-wg/S2S/sglanvil/forSanjiv/H2OSOI_E3SMLE
...
% --------- E3SMLE --------- Monthly
file='/glade/campaign/cesm/development/cross-wg/S2S/sglanvil/forSanjiv/H2OSOI_E3SMLE/v2.LR.historical_monthly-restarts_0201.elm.h0.197001-201001.nc';
time_E3SMLE=datetime('Jan/1/1999'):datetime('Jan/31/2010');
time_E3SMLE=time_E3SMLE(day(time_E3SMLE)==15);
H2OSOI=ncread(file,'H2OSOI',[1 1 349],[Inf 5 Inf]); % top levs, note 199901 start
DZSOI=ncread(file,'DZSOI',[1 1],[Inf 5])*1000; % top levs
DZSOI=repmat(DZSOI,[1,1,length(time_E3SMLE)]);
lon=ncread(file,'lon');
lat=ncread(file,'lat');
...
H2OSOI_E3SMLE=NaN(length(lon_ERA5),length(lat_ERA5),length(time_E3SMLE));
for itime=1:length(time_E3SMLE)
    itime
    H2OSOI_E3SMLE(:,:,itime)=griddata(lon,lat,...
...
for ilon=1:length(lon)
    for ilat=1:length(lat)
        if isnan(squeeze(H2OSOI_ERA5(ilon,ilat,1)))
            H2OSOI_E3SMLE(ilon,ilat,:)=NaN;
        end
    end
end

X=H2OSOI_ERA5(:,:,year(time_ERA5)>=1999 & year(time_ERA5)<=2009);
Y=H2OSOI_E3SMLE(:,:,year(time_E3SMLE)>=1999 & year(time_E3SMLE)<=2009);

%
figure
contourf(lon_ERA5,lat_ERA5,squeeze(X(:,:,33))',0:0.01:0.5,'linestyle','none');
colormap(gradsmap); colorbar; clim([0 0.5])
title('H2OSOI E3SMLE')
print('/glade/work/sglanvil/CCR/E3SM_SMYLE/H2OSOI_E3SMLE_raw','-r300','-dpng');
...
title('H2OSOI Correlation (E3SMLE vs ERA5)')
print('/glade/work/sglanvil/CCR/E3SM_SMYLE/H2OSOI_E3SMLE','-r300','-dpng');
